...
int sys_shmem(void)
{
    ...
    if ((arrayMem[shmid].mem = kalloc()) == 0) // Regresa el apuntador a una pagina
        return -1;
    //Clean the page
...
int sys_shmat(void)
{
    ...
    if (arrayMem[shmid].mem)
    {
        ...
        addr = PGROUNDUP(proc->sz); // Redondear a la siguiente pagina y obtener la direccion
        mappages(proc->pgdir, (char *)addr, PGSIZE, V2P(arrayMem[shmid].mem), PTE_W | PTE_U); // La RAM tiene un campo con banderas, como lectura escritura, etc.
                                                                                              // La RAM se puede definir como RAM de Usuario para no ser afectada por el modo kernel
        proc->sz += PGSIZE;
        switchuvm(proc); // Switch User Virtual Memory
